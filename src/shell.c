...
static void cmd_run(const char *path, const char *arg) {
    ...
    // Paso 4. Si el scheduler no está corriendo y hay procesos, arrancarlo.
...
static void cmd_ps(void) {
    ...
    // Paso 2. Si no hay procesos, informar y salir.
    if (process_count == 0) {
    ...
    // Paso 3. Imprimir la tabla de procesos.
    printf("\n");
    pcb_print_table();

    // Paso 4. Imprimir la ready queue.
    printf("\n");
    rq_print();
    ...
    for (int i = 0; i < process_count; i++) {
        if (process_table[i].pid == target_pid && process_table[i].state != PROC_TERMINATED) {
            ...
            if (i == scheduler_get_running()) {
                // Si era el proceso corriendo, despachar al siguiente
                // (el sigchld handler ya no lo hará porque ya lo recogimos)
            }
